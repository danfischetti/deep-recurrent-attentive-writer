...
class LSTM(object):
    ...
    def __init__(self, n_in, n_hidden):
        ...
        self.params = [self.wx, self.wh, self.bh,
                       self.wx_in, self.wh_in, self.b_in,
                       self.wx_out, self.wh_out, self.b_out,
                       self.wx_forget, self.wh_forget, self.b_forget,
                       self.h0]

    def recurrence(self,x_t, s_tm1, y_tm1):
           in_t = T.nnet.sigmoid(T.dot(x_t, self.wx_in)
                                + T.dot(y_tm1, self.wh_in) + self.b_in)
           out_t = T.nnet.sigmoid(T.dot(x_t, self.wx_out)
